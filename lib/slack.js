...
export function summarizeUnreads(conversations) {
  ...
  for (const c of conversations) {
    ...
    // Slack auto-creates DMs with "X joined Slack — say hello" prompts
    // (subtype joiner_notification) that its own UI never badges — don't
    // count a DM whose only unread is that prompt.
    if (unread === 1 && c.latest?.subtype === 'joiner_notification') continue;
